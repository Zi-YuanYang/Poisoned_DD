...
from models.network import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN, VGG16, ResNet34
from get_meddata import OrganAMNIST, OrganSMNIST
...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        im_size = (28,28)
        num_classes = 11
        mean = [0.46802825, 0.46802825, 0.46802825]
        std = [0.29742093, 0.29742093, 0.29742093]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = OrganAMNIST(data_path, 'train',transform)
        dst_test = OrganAMNIST(data_path,'test',transform)
        ...
        im_size = (28,28)
        num_classes = 11
        mean = [0.4953108, 0.4953108, 0.4953108]
        std = [0.28263775, 0.28263775, 0.28263775]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = OrganSMNIST(data_path, 'train',transform)
        dst_test = OrganSMNIST(data_path,'test',transform)
        class_names = []
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    if model == 'MLP':
        ...
        net = ViT(image_size, channel_size, patch_size, embed_size, num_heads, classes, num_layers, hidden_size, dropout=dropout)
    elif model == 'ConvNet':
...
def match_loss(gw_syn, gw_real, args):
    ...
    dis = torch.tensor(0.0, requires_grad=True, device=args.device)
    
    # for i, gw in enumerate(gw_real):
    #     print(f"gw_real[{i}] requires_grad: {gw.requires_grad}")
    # for i, gw in enumerate(gw_syn):
    #     print(f"gw_syn[{i}] requires_grad: {gw.requires_grad}")
    # print('13')
...
def epoch_fin(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    criterion = criterion.to(args.device)
...
def evaluate_synset_fing(it_eval, net, images_train, labels_train, testloader, args):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    lr = float(args.lr_net)
    ...
    start = time.time()
    for ep in range(Epoch+1):
        loss_train, acc_train = epoch_fin('train', trainloader, net, optimizer, criterion, args, aug = True)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)

    time_train = time.time() - start
    loss_test, acc_test = epoch_fin('test', testloader, net, optimizer, criterion, args, aug = False)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
