...
class ConvNet(nn.Module):
    def __init__(self, channel, num_classes, net_width, net_depth, net_act, net_norm, net_pooling, im_size = (32,32)):
        ...
        self.fc = nn.Linear(num_feat, 512)
        # self.fc = nn.Linear(1152, 512)
        self.classifier = nn.Linear(512, num_classes)
        # self._initialize_weights_to_zero()

    def forward(self, x):

        out = self.features(x)
        fe = out.view(out.size(0), -1)
        fe = self.fc(fe)
    ...
    # def _initialize_weights_to_zero(self):
...
class LeNet(nn.Module):
    ...
    def __init__(self, channel,num_classes):
        ...
        self.conv1 = nn.Conv2d(channel, 6, 5, padding=2 if input_size == 28 else 0)
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        x = self.features(x)
        fe = x.reshape(x.size(0), -1)
        x = self.classifier(fe)
        return x, fe
